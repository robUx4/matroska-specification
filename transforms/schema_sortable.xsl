...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:str="http://exslt.org/strings" exclude-result-prefixes="str">
  ...
  <!-- Create a version of the schema with the path without occurence as attribute, so it can be sorted -->
  ...
  <xsl:template name="parsePath">
    ...
    <xsl:attribute name="sortPath"><xsl:value-of select="$EBMLMasterPath" /></xsl:attribute>
